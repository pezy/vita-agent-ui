...
export class StreamParser {
  ...
  private currentThinkingBlock: Extract<MessageBlock, { type: "text" }> | null =
    null;
  ...
  public ttsState: { isSpeaking: boolean; text?: string; provider?: string } = {
    isSpeaking: false,
  };
  ...
  processChunk(chunk: StreamChunk): MessageBlock[] {
    if ("type" in chunk) {
      if (chunk.type === "token") {
        ...
        if (chunk.id) {
          toolBlock = this.blocks.find(
            (b) => b.type === "tool_call" && b.id === chunk.id
          );
        } else {
        ...
        const toolBlock = this.blocks.find(
          (b) => b.type === "tool_call" && b.id === chunk.id
        );
      ...
      if (this.currentThinkingBlock) {
        ...
        if (firstTagIndex !== -1) {
          ...
          this.buffer = this.buffer.substring(
            firstTagIndex + foundTag.length + 2
          ); // +2 for < and >
          // Loop continues
        } else {
